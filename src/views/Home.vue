  ...
  <div id="home">
    <section class="banner">
      ...
      <div class="banner-content container text-white text-center">
        <!-- Slogan của trường -->
        <div class="banner-slogan mb-6">
          <div class="flex items-center justify-center gap-4 mb-3">
            <span class="slogan-badge">Innovation</span>
            <span class="slogan-separator">•</span>
            <span class="slogan-badge">Pioneering</span>
            <span class="slogan-separator">•</span>
            <span class="slogan-badge">Quality</span>
          </div>
          <p class="text-[14px] text-gray-200 font-medium">
            HSB - School of Business and Management, Vietnam National University, Hanoi
          </p>
        </div>
        ...
        <div class="flex justify-center">
          <button class="flex items-center gap-3 text-[15px] bg-white text-black rounded-3xl px-6 py-2 font-semibold">
            <span class="icon">
        ...
        <div class="banner-search__input px-16 mx-14 mt-4">
          <div class="flex items-center bg-white text-black rounded-lg h-[56px] px-2">
            <span class="ps-3 pe-2">
              <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" fill="currentColor" class="bi bi-search"
                viewBox="0 0 16 16">
                <path
                  d="M11.742 10.344a6.5 6.5 0 1 0-1.397 1.398h-.001q.044.06.098.115l3.85 3.85a1 1 0 0 0 1.415-1.414l-3.85-3.85a1 1 0 0 0-.115-.1zM12 6.5a5.5 5.5 0 1 1-11 0 5.5 5.5 0 0 1 11 0" />
              </svg>
            </span>
            <input type="text" class="p-2.5 outline-0" style="flex: 1" placeholder="Search all assets"
              v-model="searchInput" @keyup.enter="handleSearch" />
            <div class="group-actions flex items-center gap-3">
              ...
              <button class="btn" style="padding: 10px; display: none">
                <span>
                  <svg xmlns="http://www.w3.org/2000/svg" viewBox="-49 141 512 512" width="20" height="20"
                    aria-hidden="true">
                    <path
        ...
        <div class="banner-slide flex">
          <div class="banner-slide__left">
            <ul class="flex flex-col items-start gap-3.5 text-[17px] font-semibold">
              <li @click="changeBannerTab(0)" class="flex items-center gap-2 cursor-pointer hover:text-[#fff]"
                :class="[bannerTabIndex === 0 ? 'text-white' : 'text-[#9c9c9c]']">
                Top
                <span class="mt-0.5"><svg xmlns="http://www.w3.org/2000/svg" viewBox="-49 141 512 512" width="17"
                    height="17" fill="#fff" aria-hidden="true">
                    <path
              ...
              <li @click="changeBannerTab(1, 'image')" class="cursor-pointer hover:text-[#fff]"
                :class="[bannerTabIndex === 1 ? 'text-white' : 'text-[#9c9c9c]']">
                Images
              </li>
              <li @click="changeBannerTab(2, 'video')" class="cursor-pointer hover:text-[#fff]"
            ...
            <div class="swiper" ref="swiperContainer">
              <div class="swiper-wrapper">
                <div class="swiper-slide" v-for="(slide, index) in slides" :key="index">
                  <a v-if="slide?.file_type === 'image' || slide?.file_type === 'template'"
                    :href="`search?query=${slide?.tag_name}&detail_id=${slide?.id}`" class="swiper-slide__link">
                  ...
                  <a v-else-if="slide?.file_type === 'video'"
                    :href="`search?query=${slide?.tag_name}&detail_id=${slide?.id}`" class="swiper-slide__link">
                    <video class="h-full w-full object-cover" @mouseover="playVideo" @mouseleave="stopVideo">
    ...
    <section class="pb-10">
      <div class="container">
        ...
        <div class="grid grid-cols-8 gap-7 mt-8">
          <a v-for="category in categories" :key="category.id" :href="'/category?menu=' +
            category.id +
    ...
    <div v-if="!user" class="container">
      <div class="flex items-center justify-center bg-[#f0f4fd] rounded-lg h-[84px]">
        ...
        <a href="/register" class="btn bg-black" style="color: #fff; padding: 10px 20px; font-size: 15px">
          Sign in now
        </a>
      </div>
    </div>

    <section class="container home-designers" style="padding-top: 60px; padding-bottom: 40px">
      <div>
        ...
        <div class="flex items-center justify-center gap-3 mt-7 mb-10">
          <template v-for="tag in tags" :key="tag.id">
            <a :href="`/search?query=${tag.name}`" class="btn" style="font-size: 12px">
              <span>
                <svg xmlns="http://www.w3.org/2000/svg" viewBox="-49 141 512 512" width="12" height="12"
                  aria-hidden="true">
                  <path
        ...
        <div class="home-designers__product grid grid-cols-7 grid-rows-3 gap-7">
          <template v-for="favorite in favorites" :key="favorite.id">
            <template v-if="favorite.file_type === 'video'">
              <a :href="`search?query=${favorite?.tag_name}&detail_id=${favorite?.id}`"
                ...
                <p class="absolute pointer-events-none z-10 flex items-end inset-0 p-3.5 text-white text-[15px]">
                  {{ favorite.title }}
                </p>
            ...
            ">
              <a :href="`search?query=${favorite?.tag_name}&detail_id=${favorite?.id}`"
                class="home-designers__product--item relative h-[220px] rounded-lg font-bold overflow-hidden">
                <img class="hover:scale-105 w-full h-full object-cover"
                  style="transition: scale 0.3s linear; width: 100%" :src="favorite.file_url" />
                <div class="absolute inset-0 pointer-events-none"></div>
                <p class="absolute pointer-events-none z-10 flex items-end inset-0 p-3.5 text-white text-[15px]">
                  {{ favorite.title }}
                </p>
          ...
          <div class="flex flex-col gap-6">
            <div v-for="itemNew in resourceNewResults?.[0]" :key="itemNew.id">
              <template v-if="itemNew.file_type === 'video'">
                <a :href="`search?query=${itemNew?.tag_name}&detail_id=${itemNew?.id}`"
                  class="home-inspired__item relative overflow-hidden">
                  <video class="item-preview h-full w-full object-cover rounded-md" @mouseover="playVideo"
              ...
              ">
                <a :href="`search?query=${itemNew?.tag_name}&detail_id=${itemNew?.id}`"
                  class="home-inspired__item relative overflow-hidden">
                  <img class="item-preview rounded-md w-full" :src="itemNew.file_url" :alt="itemNew.title" />
                  <div
                    class="absolute flex flex-col justify-end inset-0 pointer-events-none rounded-md p-3.5 text-white">
                    <p class="home-inspired__text">
                      {{ itemNew.description }}
    ...
    <section class="container">
      <div style="padding: 80px 80px 120px 80px">
        <div class="text-center font-semibold">
          ...
          <a v-if="!user?.id" href="/register" class="w-fit btn bg-black mx-auto mt-6 mb-2.5"
